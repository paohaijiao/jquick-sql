...
public class JQuickSQL {
    ...
    public JQuickSQL withWorker(String host, int port) {
        List<WorkerEndpoint> endpoints = new ArrayList<>(config.getRuntime().getWorkers() != null ? config.getRuntime().getWorkers() : new ArrayList<>());
        endpoints.add(new WorkerEndpoint("worker-" + (endpoints.size() + 1), host, port, endpoints.size()));
    ...
    public JQuickSQL withWorkers(List<String> addresses) {
        ...
        for (int i = 0; i < addresses.size(); i++) {
            ...
            endpoints.add(new WorkerEndpoint("worker-" + (i + 1), host, port, i));
        }
        config.getRuntime().setWorkers(endpoints);
        return this;
    }
...
//        fragmenter.printFragments(distributedPlan);
    ...
    private void startEmbeddedWorkers() {
        ...
        for (int i = 0; i < workerCount; i++) {
            ...
            workers.add(worker);
            endpoints.add(new WorkerEndpoint("worker-" + (i + 1), "localhost", port, i));
            try {
                Thread.sleep(200);
