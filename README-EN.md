...
JQuickSQL is a lightweight SQL query engine that supports standard SQL syntax and OLAP operations, suitable for querying
and analyzing in-memory datasets (such as `DataSet`).
...
The following aggregate functions are supported by default. For extending aggregate functions, please refer to the
registerFunction method of JAggregateFunctionFactory.
