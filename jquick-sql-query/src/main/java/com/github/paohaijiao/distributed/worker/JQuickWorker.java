...
public class JQuickWorker {
    ...
    public JQuickRegisterWorkerResponse registerWithCoordinator(String host, int port) {
        ...
        try {
            ...
            if (response.getSuccess()) {
                List<WorkerEndpoint> endpoints = response.getAllWorkersList().stream()
                        .map(p -> new WorkerEndpoint(p.getWorkerId(), p.getHost(), p.getPort(), p.getIndex()))
    ...
    public void updateWorkerEndpoints(List<JQuickWorkerEndpointProto> endpoints) {
        List<WorkerEndpoint> workerEndpoints = endpoints.stream()
                .map(p -> new WorkerEndpoint(
                        p.getWorkerId(),
                        p.getHost(),
                        p.getPort(),
                        p.getIndex()
    ...
    public List<JQuickWorkerEndpointProto> getAllWorkerEndpoints() {
        ...
        // 返回当前已知的所有 Worker 端点
        for (WorkerEndpoint endpoint : partitionManager.getWorkerEndpoints()) {
